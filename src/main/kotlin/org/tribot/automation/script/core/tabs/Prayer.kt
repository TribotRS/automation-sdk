...
interface Prayer {
    fun isActive(prayer: PrayerType): Boolean
    fun set(prayer: PrayerType, active: Boolean): Boolean
    fun getActive(): List<PrayerType>
    fun toggleQuickPrayers(): Boolean
    fun isQuickPrayerActive(): Boolean
}

enum class PrayerType(val level: Int) {
    THICK_SKIN(1),
    BURST_OF_STRENGTH(4),
